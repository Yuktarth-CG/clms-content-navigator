...
const mockFetchSchoolAndStudents = async (udise: string) => {
  ...
  const students: MockStudent[] = Array.from({ length: 12 }).map((_, i) => ({
    id: `stu-${i + 1}`,
    name: `Student ${i + 1}`,
    grade: 5,
  }));
  const assessments = [
    { id: "assess-lep", name: "LEP Diagnostic Assessment" },
    { id: "assess-mid", name: "Mid-term Diagnostic" },
  ];
  return { school, students, assessments };
};

const questionTypeOptions: { key: QuestionTypeOption; label: string }[] = [
...
const StepHeader = ({ step, label, Icon }: { step: number; label: string; Icon: React.ComponentType<any> }) => (
...
const PersonalizedWorksheet: React.FC = () => {
  ...
  const handleFetch = async () => {
    ...
    setLoading(true);
    try {
      const { school, students, assessments } = await mockFetchSchoolAndStudents(udise);
      setSchool(school);
      setStudents(students);
      setAssessments(assessments);
      toast({ title: "Mock data loaded", description: `Found ${students.length} students for UDISE ${udise}.` });
      setStep(2);
    } finally {
      setLoading(false);
    }
    ...
    <div className="space-y-6">
      <Card>
        ...
        <CardContent className="space-y-8">
          ...
          {step === 1 && (
            <Section
              title="Enter School UDISE"
              description="We will fetch mock school details and students for this UDISE."
            >
          ...
          {step === 2 && (
            <Section
              title="Review School & Students"
              description="Verify the mock school details and continue."
            >
          ...
          {step === 3 && (
            <Section
              ...
              description="Choose the past assessment to base the personalised worksheets on."
            >
              <div className="space-y-2">
          ...
          {step === 4 && (
            <Section
              ...
              description="Pick a method and select the students to include."
            >
          ...
          {step === 6 && (
            <Section
              ...
              description="Worksheets were generated locally in this demo. In production, PDFs will be saved to storage for download and printing."
            >
              <Summary />
          ...
          <div className="flex items-center justify-between pt-2">
            ...
            {step < 5 && (
              <Button onClick={() => setStep((s) => s + 1)} disabled={!canGoNext || loading}>
                Next
              </Button>
            )}
            {step === 5 && (
              <Button onClick={handleGenerate} disabled={!canGoNext || loading}>
